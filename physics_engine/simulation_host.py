...
FW_M_CMD_FULL_SCALE_DEFAULT = 8.0
FW_TAU_FULL_SCALE_DEFAULT = 0.05
FW_TELEMETRY_VERSION_DEFAULT = 1
...
def _eval_c_float_expr(expr):
    if not expr:
        return None
    cleaned = re.sub(r"(?<=\d)[fFuUlL]+\b", "", expr)
...
def _eval_c_int_expr(expr):
    parsed_float = _eval_c_float_expr(expr)
...
def read_firmware_settings():
    settings = {
        "max_voltage": FW_MAX_VOLTAGE_DEFAULT,
        "dipole_strength": FW_DIPOLE_STRENGTH_DEFAULT,
        "m_cmd_full_scale": FW_M_CMD_FULL_SCALE_DEFAULT,
        "tau_full_scale": FW_TAU_FULL_SCALE_DEFAULT,
        "telemetry_version": FW_TELEMETRY_VERSION_DEFAULT,
    ...
    try:
        ...
        expr_m_scale = _extract_define_expr(text, "M_CMD_FULL_SCALE_AM2")
        expr_tau_scale = _extract_define_expr(text, "TAU_FULL_SCALE_NM")
        expr_version = _extract_define_expr(text, "TELEMETRY_PACKET_VERSION")

        parsed_max = _eval_c_float_expr(expr_max)
        parsed_mtq = _eval_c_float_expr(expr_mtq)
        parsed_m_scale = _eval_c_float_expr(expr_m_scale)
        parsed_tau_scale = _eval_c_float_expr(expr_tau_scale)
        parsed_version = _eval_c_int_expr(expr_version)

        if parsed_max and parsed_max > 0.0:
            settings["max_voltage"] = parsed_max
        if parsed_mtq and abs(parsed_mtq) > 1e-12:
            settings["dipole_strength"] = 1.0 / parsed_mtq
        if parsed_m_scale and parsed_m_scale > 0.0:
            settings["m_cmd_full_scale"] = parsed_m_scale
        if parsed_tau_scale and parsed_tau_scale > 0.0:
            settings["tau_full_scale"] = parsed_tau_scale
        if parsed_version is not None and parsed_version >= 0:
            settings["telemetry_version"] = parsed_version

        settings["source"] = FW_CONFIG_PATH
    except OSError:
        pass
...
def get_args():
    ...
    parser.add_argument("--scenario", choices=["detumble", "pointing"], default="detumble", help="Simulation scenario")
    parser.add_argument("--initial_omega", nargs=3, type=float, metavar=("WX", "WY", "WZ"), help="Initial angular velocity [rad/s]")
    parser.add_argument("--duration", type=float, default=0, help="Simulation duration in seconds (0 = infinite)")
    parser.add_argument("--quiet", action="store_true", help="Suppress console output")
    parser.add_argument("--realtime", action="store_true", help="Run simulation at 1x speed")
    parser.add_argument("--open-loop", action="store_true", help="Bypass PI controller on firmware (V=I*R)")
    parser.add_argument("--force-mode", choices=["auto", "detumble", "spin", "pointing"], default="auto", help="Force STM32 outer-loop mode")
    parser.add_argument(
        "--reset-controller",
    ...
    parser.add_argument("--dt", type=float, help="Physics step size in seconds (overrides realtime/fast defaults)")

    # Controller Gains (sent to firmware for runtime tuning)
    parser.add_argument("--kbdot", type=float, default=400000.0, help="B-dot gain for detumble mode (positive = damping)")
    parser.add_argument("--kp", type=float, default=0.0012, help="Proportional gain for pointing mode")
    parser.add_argument("--ki", type=float, default=3e-05, help="Integral gain for pointing mode")
    ...
    # Physics Calibration
    parser.add_argument("--max-voltage", type=float, default=None, help="Max H-bridge power supply voltage [V] (optional override)")
    parser.add_argument("--dipole-strength", type=float, default=None, help="Magnetorquer strength factor [Am^2/A] (optional override)")
    parser.add_argument(
        "--structural-config",
        type=str,
        default=DEFAULT_STRUCTURAL_CONFIG_PATH,
        help="Path to structural constants JSON",
    )
...
def main():
    ...
    fw_dipole_strength = firmware_settings["dipole_strength"]
    fw_m_cmd_full_scale = firmware_settings["m_cmd_full_scale"]
    fw_tau_full_scale = firmware_settings["tau_full_scale"]
    fw_telemetry_version = firmware_settings["telemetry_version"]

    try:
        structural_constants = load_structural_constants(args.structural_config)
    except Exception as exc:
        print(f"ERROR: failed to load structural constants: {exc}", file=sys.stderr)
        sys.exit(2)
    ...
    if not args.quiet:
        ...
        print(f"Firmware telemetry scales: MFS={fw_m_cmd_full_scale} Am^2, TFS={fw_tau_full_scale} Nm, Ver={fw_telemetry_version}")
        print(f"Firmware settings source: {firmware_settings['source']}")
        print(f"Structural config: {args.structural_config}")
        print(
            "W26 orbit: "
            f"a={structural_constants.semimajor_axis_m:.1f} m, "
            f"inc={structural_constants.inclination_deg:.4f} deg, "
            f"period={structural_constants.period_s:.2f} s, "
            f"RAAN={structural_constants.raan_deg:.2f} deg"
        )
        print(f"Constants source doc: {structural_constants.source_doc_path}")
        print(f"Force mode: {args.force_mode}")

    # Initialize components
    sat = SatellitePhysics(
        max_voltage=eff_max_voltage,
    ...
    if args.initial_omega:
        ...
        w_init = np.array([0.5, 0.5, 0.5])
    else:  # pointing
        w_init = np.array([0.05, 0.05, 0.05])
    ...
    if args.dt:
        dt_val = args.dt
        if not args.quiet:
            print(f"Running with CUSTOM step size: {dt_val}s")
    elif args.realtime:
        dt_val = 1.0
        if not args.quiet:
            print("Running in REALTIME mode (1s step)")
    else:
        dt_val = 0.1
        if not args.quiet:
            print("Running in FAST mode (0.1s step, ~10 mins per orbit)")
    ...
    m_cmd_fw = np.array([0.0, 0.0, 0.0])
    tau_raw_fw = np.array([0.0, 0.0, 0.0])
    tau_proj_fw = np.array([0.0, 0.0, 0.0])
    telemetry_flags = 0
    ...
    fw_mtq_dipole_to_amp = 1.0 / max(fw_dipole_strength, 1e-9)
    ...
    try:
        while True:
            ...
            if args.duration > 0 and sim_time >= args.duration:
                ...
                mode_dwell_ratio = mode_counts.get(final_mode, 0) / total_mode_samples if total_mode_samples > 0 else 0.0
                forced_mode_dwell = (
                    mode_counts.get(forced_mode_code, 0) / total_mode_samples
                    if forced_mode_code != 0 and total_mode_samples > 0
                    else -1.0
                )
                sat_voltage_ratio = sat_axis_count / sat_axis_samples if sat_axis_samples > 0 else 0.0
                avg_proj_loss = proj_loss_sum / proj_loss_samples if proj_loss_samples > 0 else 0.0
                int_clamp_ratio = int_clamp_count / int_clamp_samples if int_clamp_samples > 0 else 0.0
            ...
            sat.rk4_step(sim_time, dt_step, firmware_voltages, b_inertial)

            q = sat.state[0:4]
            w = sat.state[4:7]
            sat_currents = sat.state[7:10]

            # Read sensors (post-update to avoid skew)
            ...
            if response:
                firmware_voltages = np.array(response["command_voltage"])
                packed_mode = int(response["packed_mode"]) & 0xFF
                adcs_mode_id = packed_mode & 0x03
                int_clamp_flag = (packed_mode >> 2) & 0x01
                proj_loss_q5 = (packed_mode >> 3) & 0x1F
                proj_loss = proj_loss_q5 / 31.0
                m_cmd_fw = np.array(response["m_cmd"])
                tau_raw_fw = np.array(response["tau_raw"])
                tau_proj_fw = np.array(response["tau_proj"])
                telemetry_flags = int(response["telemetry_flags"]) & 0xFF
            ...
            telemetry_skip = max(1, int(0.1 / dt_step))

            if not args.quiet and step_count % telemetry_skip == 0:
                m_applied = sat_currents * sat.dipole_strength
                torque_applied = np.cross(m_applied, b_body)
                i_target = m_cmd_fw * fw_mtq_dipole_to_amp
                m_str = tel.mode_map.get(adcs_mode_id, str(adcs_mode_id))

                tel.send_telemetry(
                    w=w,
                    q=q,
                    tau_applied=torque_applied,
                    sim_time=sim_time,
                    adcs_mode_id=adcs_mode_id,
                    target_i=i_target,
                    actual_i=sat_currents,
                    pwm_cmd=firmware_voltages,
                    b_body=b_body,
                    b_eci=b_inertial,
                    m_cmd_fw=m_cmd_fw,
                    m_applied=m_applied,
                    tau_raw_fw=tau_raw_fw,
                    tau_proj_fw=tau_proj_fw,
                    proj_loss_fw=proj_loss_q5 / 31.0 if response else 0.0,
                    int_clamp_fw=int_clamp_flag if response else 0,
                ...
                tau_raw_eci = r_body_to_inertial @ tau_raw_fw
                ...
                if step_count % (telemetry_skip * 10) == 0:
                    print(
                        f"t={sim_time:5.1f}s | Mode={m_str:10s} | W={np.linalg.norm(w):.3f} "
                        f"| Err={angle_err:5.1f}° | {stability_str} (P={work_rate:.2e}) "
                        f"| V={firmware_voltages}"
                    )
                    if args.debug:
                        print(
                            f"  DEBUG: w={w} | B={b_body} | m_cmd={m_cmd_fw} | "
                            f"tau_raw={tau_raw_fw} | tau_proj={tau_proj_fw}"
                        )
            ...
            if not args.quiet:
                time.sleep(max(0.0, dt_step - t_elapsed))
