...
HAL_StatusTypeDef ADT7420_ReadCelsius(ADT7420_Handle *h, float *temp_c)
{
    ...
    if (cfg & 0x80)
        *temp_c = ((float)raw) / 128.0f; // 16-bit mode
    else
        *temp_c = ((float)(raw >> 3)) / 16.0f; // 13-bit default
    return HAL_OK;
}
...
{
    HAL_StatusTypeDef status = ADT7420_ReadCelsius(h, &data->temperature_c);
    data->timestamp_ms = HAL_GetTick();
    data->valid = (status == HAL_OK);
    return status;
}