...
static int AppManualValidation_IsActive(void)
{
    manual_validation_state_t state;
    AppManualValidation_Snapshot(&state);
    return ManualValidation_IsOverrideActive(&state);
}
...
static void app_manual_validation_step(void)
{
    ...
    AppManualValidation_Snapshot(&state);
    if (ManualValidation_IsOverrideActive(&state) == 0) {
        return;
    }

    ManualValidation_ComputeOutput(&state, HAL_GetTick(), &output);
    InnerLoop_SetTargetCurrent(output.target_current_x,
                               output.target_current_y,
                               output.target_current_z);
}
...
void AppRuntime_OnConsoleByteFromISR(uint8_t byte)
{
    ...
    if (xQueueSendFromISR(g_console_rx_queue, &byte, &higher_priority_task_woken) != pdPASS) {
        g_console_rx_drop_count++;
    }
...
{
    char line[CONSOLE_LINE_MAX_LEN + 1U];
    size_t len = 0U;
    uint8_t byte = 0U;
    (void)argument;
    ...
    for (;;) {
        ...
        if (byte == '\n') {
            ...
            log_printf_dma("CMD %s :: %s drops=%lu",
                           result.message,
                           status,
                           (unsigned long)g_console_rx_drop_count);
            len = 0U;
            continue;
        }

        if (len < CONSOLE_LINE_MAX_LEN) {
            line[len++] = (char)byte;
        } else {
            len = 0U;
            log_printf_dma("CMD line too long; dropped");
        }
    }
}
...
static void ADCSTask(void *argument)
{
    ...
    for (;;) {
        ...
        if (AppManualValidation_IsActive() != 0) {
            continue;
        }
